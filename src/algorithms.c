...
mean_local_threshold(const Image *in_img, const int win_size, const int c)
{
    ...
    int_data = compute_integral_image(in_img);
    if (NULL == int_data) {
	...
	for (ic = 0; ic < num_cols; ic++) {
		...
		mean = (int_data[(ir+half_ws)*num_cols+ic+half_ws] + int_data[(ir-half_ws)*num_cols+ic-half_ws] - int_data[(ir-half_ws)*num_cols+ic+half_ws] - int_data[(ir+half_ws)*num_cols+ic-half_ws]) / (win_size * win_size);
	    }
